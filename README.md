...
后续可根据实际 Excel 模板调整。初版建议字段如下：

| 字段名          | 是否必填 | 说明                             |
| --------------- | -------- | -------------------------------- |
| 用例编号        | 是       | 唯一标识，例如 TC_VOLUME_001     |
| 用例名称        | 是       | 用例标题                         |
| 所属模块        | 否       | 例如卷管理、存储池管理           |
| 关联接口        | 是       | 例如 POST /api/v1/volumes        |
| 请求方法        | 否       | GET / POST / PUT / DELETE        |
| 用例类型        | 是       | 正向 / 反向 / 边界 / 权限 / 流程 |
| 优先级          | 是       | P0 / P1 / P2 / P3                |
| 标签            | 否       | 冒烟、回归、高危等               |
| 前置条件        | 否       | 执行前要求                       |
| 步骤编号        | 是       | 流程型用例需要                   |
| 操作步骤        | 是       | 当前步骤描述                     |
| 请求 Header     | 否       | JSON 格式                        |
| 请求 Path 参数  | 否       | JSON 格式                        |
| 请求 Query 参数 | 否       | JSON 格式                        |
| 请求 Body       | 否       | JSON 格式                        |
| 预期状态码      | 是       | 例如 200、201、400               |
| 预期响应        | 否       | JSON 或自然语言                  |
| 断言规则        | 否       | JSONPath / 正则 / Schema         |
| 提取变量        | 否       | 从响应中提取变量                 |
| 后置清理        | 否       | 清理动作                         |
| 是否自动化      | 是       | 是 / 否                          |
| 备注            | 否       | 补充说明                         |
...
1. API 项目管理。
2. 原始 API 文档上传。
3. OpenAPI / Swagger JSON 导入。
4. AI 解析 API 文档并生成标准 OpenAPI JSON。
5. 人工审核标准 API 文档。
6. 发布测试集。
7. Excel 测试用例导入。
8. 用例与 API 关联。
